...
class WQLinear_GEMM_Triton(nn.Module):
    def __init__(
        ...
        assert in_features % (32 // self.w_bit) == 0

        self.register_buffer(
            "qweight",
            torch.zeros(
                (in_features // (32 // self.w_bit), out_features),
                dtype=torch.int32,
                device=dev,
            ),
        )
    ...
    def from_linear(
        ...
        qweight = torch.zeros(
            (intweight.shape[0] // (32 // awq_linear.w_bit), intweight.shape[1] ),
            dtype=torch.int32,
        ...
        for row in range(intweight.shape[0] // pack_num):
            for i in range(pack_num):
                qweight_row = intweight[row * pack_num + i]
                qweight[row] |= qweight_row << (i * awq_linear.w_bit)
